...
const getDefaultDateRange = (granularity: TimeGranularity) => {
    ...
    switch (granularity) {
        ...
        case TimeGranularity.YEARLY:
            return {
                startDate: new Date(year - 4, 0, 1).toISOString().split('T')[0],
                endDate: new Date(year, 11, 31).toISOString().split('T')[0]
            };
...
// Dopracowane Style Components - bez przeskoków UI
...
/* KLUCZOWE: Stała wysokość sekcji filtrów - bez przeskoków */
const FiltersSection = styled.div`
    padding: ${theme.spacing.md} ${theme.spacing.xl};
    background: ${theme.surfaceAlt};
    border-bottom: 1px solid ${theme.border};
    min-height: 80px; /* Stała wysokość */
    display: flex;
    align-items: center;
    flex-shrink: 0;
`;
...
const FilterLabel = styled.label`
    ...
    color: ${theme.text.secondary};
    white-space: nowrap;
    min-width: 50px; /* Stała szerokość */
`;

/* KLUCZOWE: Stały kontener dla pól dat - bez przeskoków */
const DateInputsContainer = styled.div`
    min-width: 300px; /* Stała minimalna szerokość */
    min-height: 40px; /* Stała wysokość */
    display: flex;
    align-items: center;
