...
const KPICard: React.FC<KPICardProps> = ({
    ...
    if (prevValue !== undefined && prevValue !== null) {
        // Konwersja do liczb - najpierw należy obsłużyć przypadek string
        let cleanValue: number;
        let cleanPrevValue: number;

        // Funkcja pomocnicza do czyszczenia wartości
        const cleanValueString = (val: string | number): string => {
            if (typeof val === 'string') {
                return val.replace(/[^\d.-]/g, ''); // Usuwa wszystko poza cyframi, kropką i minusem
            }
            return val.toString();
        };

        if (typeof value === 'string') {
            cleanValue = parseFloat(cleanValueString(value));
        } else {
            cleanValue = value;
        }

        if (typeof prevValue === 'string') {
            cleanPrevValue = parseFloat(cleanValueString(prevValue));
        } else {
            cleanPrevValue = prevValue;
        }

        // Sprawdzamy, czy wartości są liczbami (nie NaN)
        if (!isNaN(cleanValue) && !isNaN(cleanPrevValue) && cleanPrevValue !== 0) {
